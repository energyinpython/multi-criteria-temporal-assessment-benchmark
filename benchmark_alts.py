...
def main():
    ...
    crits_str = [str(el) for el in crits]
    alt_str = [str(el) for el in alters]
    ...
    for i in range(iterations):
        # loop with Alternatives
        for it, m in enumerate(alters):
    ...
    df_final['Alternatives'] = x1
    df_final['Correlation'] = y
    df_final['Methods'] = w
    df_final.to_csv('benchmarkable/data_alts.csv')
    df_final = df_final.rename_axis('Lp')
    df_final.to_csv('benchmarkable/dataLp_alts.csv')


    # plot visualization
    sns.set_style("darkgrid")
    plt.figure(figsize = (11, 5))
    ax = sns.boxenplot(x = 'Alternatives', y = 'Correlation', hue = 'Methods', data = df_final)
    ...
    plt.tight_layout()
    plt.savefig('benchmarkable/boxenplot_alts.pdf')
    plt.show()
